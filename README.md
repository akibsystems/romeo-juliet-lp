...
ブラウザごとに乱数ID(`rj-<uuid>`)を生成しlocalStorageに保存。

- Stripeリンクに `?client_reference_id=<ID>` を付与 → 決済データに記録
- フォームの「クライアントID」項目に同じIDを自動送信
- 突合: Stripeの支払いエクスポート(client_reference_id列)×フォーム回答シートをIDでVLOOKUP
- GA4の`support_click`にも`client_id`を付与(押したが未決済の分析用)
